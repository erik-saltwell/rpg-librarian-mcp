...
tested, used by `scan`'s barcode step; having two ISBN
...
_GOOGLE_BOOKS_USER_AGENT = "rpg-librarian (gzip)"
...
class IsbnLookupResult:
    ...
    provider: str | None = None  # which service found it
...
def lookup(isbn: str) -> IsbnLookupResult | None:
    ...
    match via `log_call_fields`/`log_file_fields` -- whichever of a
    call-level or file-level wide event is currently in flight, or
    both, or neither (a no-op outside of a tracked call or file).
    """
...
def _report_lookup_stats(call_counts: dict[str, int], found_via: str | None) -> None:
    ...
    log_call_fields(**fields)
    log_file_fields(**fields)
