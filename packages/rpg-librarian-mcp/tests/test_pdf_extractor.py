...
from rpg_librarian_mcp.infrastructure.isolated_worker import SynchronousWorkerPool
...
class _Doc:
    ...
    def __init__(self) -> None:
        ...
        self._page_rects: dict[int, SimpleNamespace] = {}

    def __getitem__(self, index):
        text = self._page_texts.get(index, "extractable text " * 2)
        rect = self._page_rects.get(index, SimpleNamespace(width=612, height=792))
        return SimpleNamespace(
            get_text=lambda mode: text, rect=rect, get_images=lambda full: []
        )
...
def test_likely_image_only_is_true_for_a_multi_sheet_map_pack_without_using_the_pool(
    monkeypatch,
):
    _no_media_info(monkeypatch)
    doc = _Doc()
    doc._page_texts = {0: "", 1: "", 2: ""}
    doc._page_rects = {i: SimpleNamespace(width=1500, height=2000) for i in range(3)}
    monkeypatch.setattr(pdf_extractor.fitz, "open", lambda file_path: doc)

    def _fail_if_called(file_path):
        raise AssertionError("must not OCR a multi-sheet map pack")
