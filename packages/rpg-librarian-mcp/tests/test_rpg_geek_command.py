...
import pytest
from rpg_librarian_tools.rpggeek.client import Candidate, ProductDetails
...
async def test_search_rpg_geek_maps_candidates_to_product_candidate():
    ...
    client.search.assert_awaited_once_with("Call of Cthulhu", 5)
...
async def test_search_rpg_geek_passes_through_max_values():
    ...
    client.search.assert_awaited_once_with("x", 3)
...
async def test_search_rpg_geek_tries_isbn_then_falls_back_to_name():
    ...
    client.search.side_effect = [
        [],
        [Candidate(rpggeek_id=123, name="Fallback result")],
    ]

    result = await SearchRpgGeekCommand(client).run("Fallback", "9780306406157")

    assert [call.args for call in client.search.await_args_list] == [
        ("9780306406157", 5),
        ("Fallback", 5),
    ]
    assert result[0].source_id == "123"
