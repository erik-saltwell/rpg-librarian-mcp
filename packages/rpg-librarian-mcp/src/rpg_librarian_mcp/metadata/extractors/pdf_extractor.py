...
from rpg_librarian_tools._text_extraction import _all_pages_oversized_and_textless
...
def is_likely_image_only_isolated(path: Path) -> bool:
    """Application adapter around the public worker-safe PDF assessment."""
    return assess_images(path).status is ImageAssessmentStatus.IMAGE_ONLY
...
class PdfExtractor(MetadataExtractor):
    ...
    def _get_likely_image_only(self, page_count: int) -> bool | None:
        if self._doc.needs_pass or page_count == 0:
            return None
        if _all_pages_oversized_and_textless(self._doc):
            return True
        if page_count != 1:
            return False
