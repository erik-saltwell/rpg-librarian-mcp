...
def _sample_text_json(page_texts: dict[int, str]) -> str:
    return json.dumps(
        {"pages": {str(number + 1): text for number, text in page_texts.items()}}
    )
...
class ReadPdfsCommand(UpdateBaseCommand):
    ...
    async def process(
        ...
        """Stash the image-only policy, then process the requested entries.
    ...
    def _process_normal(
        ...
        page_text_result = self._pdf_pool.submit(
            extract_text, file_path, _text_pages(page_count)
        )
        page_texts = {page.page: page.text for page in page_text_result.pages}
        ...
        pdf_isbn = (
            barcode_match.value
            if barcode_match is not None and barcode_match.kind == "isbn"
            else None
        )
        pdf_issn = (
            barcode_match.value
            if barcode_match is not None and barcode_match.kind == "issn"
            else None
        )
        text_identifiers = find_publication_identifiers(content)
        if pdf_isbn is None and pdf_issn is None and text_identifiers:
            first = text_identifiers[0]
            pdf_isbn = first.value if first.kind == "isbn" else None
            pdf_issn = first.value if first.kind == "issn" else None
        ...
        pdf_contents = PdfContents(
            entry_id=entry.id,
            barcode=barcode_match.raw_value if barcode_match else None,
            isbn=pdf_isbn,
            issn=pdf_issn,
            sample_text=_sample_text_json(page_texts),
        )
        ...
        if llm_judged:
            judgment = judge_pdf_contents(_sample_text_json(page_texts))
