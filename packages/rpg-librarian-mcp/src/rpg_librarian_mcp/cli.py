...
def cmd_search_rpg_geek(args: argparse.Namespace, catalog: Catalog) -> None:
    command = SearchRpgGeekCommand(os.environ.get("RPGGEEK_BEARER_TOKEN") or None)
...
def cmd_lookup_rpg_geek_product(args: argparse.Namespace, catalog: Catalog) -> None:
    command = LookupRpgGeekProductCommand(
        os.environ.get("RPGGEEK_BEARER_TOKEN") or None
    )
...
def cmd_search_dtrpg(args: argparse.Namespace, catalog: Catalog) -> None:
    ...
    command = SearchDtrpgCommand(api_key)
    _print(asyncio.run(command.run(args.query, args.scope, args.max_values)))
